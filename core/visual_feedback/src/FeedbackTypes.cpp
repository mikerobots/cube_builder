...
// Face implementation
Face::Face(const Math::IncrementCoordinates& voxelPos, VoxelData::VoxelResolution res, FaceDirection dir)
...
Face Face::GroundPlane(const Math::WorldCoordinates& hitPoint) {
    Face face;
    face.m_valid = true;
    ...
    // Convert hit point to nearest increment position for voxel position
    // This helps with face ID generation - use the new coordinate converter
    face.m_voxelPosition = Math::CoordinateConverter::worldToIncrement(hitPoint);
...
Math::WorldCoordinates Face::getWorldPosition() const {
    // Enhancement: Handle ground plane
    if (m_isGroundPlane) {
        return m_groundHitPoint;
    }
    
    // Convert increment coordinates to world coordinates using the new converter
    Math::WorldCoordinates baseWorldPos = Math::CoordinateConverter::incrementToWorld(m_voxelPosition);
    float voxelSize = getVoxelSize();
    
    // Offset to face center from voxel center
    ...
    return Math::WorldCoordinates(baseWorldPos.value() + offset);
}
...
std::array<Math::WorldCoordinates, 4> Face::getCorners() const {
    float voxelSize = getVoxelSize();
    
    // Convert increment coordinates to world coordinates using the new converter
    Math::WorldCoordinates baseWorldPos = Math::CoordinateConverter::incrementToWorld(m_voxelPosition);
    Math::Vector3f basePos = baseWorldPos.value();
    
    std::array<Math::WorldCoordinates, 4> corners;
    
    switch (m_direction) {
        case FaceDirection::PositiveX:
            corners[0] = Math::WorldCoordinates(basePos + Math::Vector3f(voxelSize, 0, 0));
            corners[1] = Math::WorldCoordinates(basePos + Math::Vector3f(voxelSize, voxelSize, 0));
            corners[2] = Math::WorldCoordinates(basePos + Math::Vector3f(voxelSize, voxelSize, voxelSize));
            corners[3] = Math::WorldCoordinates(basePos + Math::Vector3f(voxelSize, 0, voxelSize));
            break;
        case FaceDirection::NegativeX:
            corners[0] = Math::WorldCoordinates(basePos + Math::Vector3f(0, 0, 0));
            corners[1] = Math::WorldCoordinates(basePos + Math::Vector3f(0, 0, voxelSize));
            corners[2] = Math::WorldCoordinates(basePos + Math::Vector3f(0, voxelSize, voxelSize));
            corners[3] = Math::WorldCoordinates(basePos + Math::Vector3f(0, voxelSize, 0));
            break;
        case FaceDirection::PositiveY:
            corners[0] = Math::WorldCoordinates(basePos + Math::Vector3f(0, voxelSize, 0));
            corners[1] = Math::WorldCoordinates(basePos + Math::Vector3f(voxelSize, voxelSize, 0));
            corners[2] = Math::WorldCoordinates(basePos + Math::Vector3f(voxelSize, voxelSize, voxelSize));
            corners[3] = Math::WorldCoordinates(basePos + Math::Vector3f(0, voxelSize, voxelSize));
            break;
        case FaceDirection::NegativeY:
            corners[0] = Math::WorldCoordinates(basePos + Math::Vector3f(0, 0, 0));
            corners[1] = Math::WorldCoordinates(basePos + Math::Vector3f(0, 0, voxelSize));
            corners[2] = Math::WorldCoordinates(basePos + Math::Vector3f(voxelSize, 0, voxelSize));
            corners[3] = Math::WorldCoordinates(basePos + Math::Vector3f(voxelSize, 0, 0));
            break;
        case FaceDirection::PositiveZ:
            corners[0] = Math::WorldCoordinates(basePos + Math::Vector3f(0, 0, voxelSize));
            corners[1] = Math::WorldCoordinates(basePos + Math::Vector3f(voxelSize, 0, voxelSize));
            corners[2] = Math::WorldCoordinates(basePos + Math::Vector3f(voxelSize, voxelSize, voxelSize));
            corners[3] = Math::WorldCoordinates(basePos + Math::Vector3f(0, voxelSize, voxelSize));
            break;
        case FaceDirection::NegativeZ:
            corners[0] = Math::WorldCoordinates(basePos + Math::Vector3f(0, 0, 0));
            corners[1] = Math::WorldCoordinates(basePos + Math::Vector3f(0, voxelSize, 0));
            corners[2] = Math::WorldCoordinates(basePos + Math::Vector3f(voxelSize, voxelSize, 0));
            corners[3] = Math::WorldCoordinates(basePos + Math::Vector3f(voxelSize, 0, 0));
            break;
    }
...
Math::WorldCoordinates Face::getCenter() const {
    auto corners = getCorners();
    Math::Vector3f center(0, 0, 0);
    for (const auto& corner : corners) {
        center = center + corner.value();
    }
    return Math::WorldCoordinates(center * 0.25f);
}
