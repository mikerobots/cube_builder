...
protected:
    ...
    void SetUp() override {
        ...
        feedbackRenderer = std::make_unique<FeedbackRenderer>(renderEngine.get());
    }
...
TEST_F(RenderStressTest, FindMaximumVoxelCount) {
    ...
    camera.setPosition(Math::WorldCoordinates(Vector3f(20.0f, 20.0f, 20.0f)));
    camera.setTarget(Math::WorldCoordinates(Vector3f(0.0f, 0.0f, 0.0f)));
    camera.setAspectRatio(16.0f/9.0f);
    
    for (int count : testCounts) {
        // Clear previous voxels in all grids
        for (int i = 0; i < 10; ++i) {
            VoxelGrid* grid = voxelData->getGrid(static_cast<VoxelResolution>(i));
            if (grid) {
                grid->clear();
            }
        }
...
TEST_F(RenderStressTest, ComplexSceneRendering) {
    ...
    Face highlightFace(Vector3i(0, 0, 0), VoxelResolution::Size_8cm, 
    ...
    double avgTime = std::accumulate(frameTimes.begin(), frameTimes.end(), 0.0) 
                    / frameTimes.size();
    const Rendering::RenderStats& stats = renderEngine->getRenderStats();
