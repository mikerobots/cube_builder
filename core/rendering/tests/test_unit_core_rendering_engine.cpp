...
// Use OrbitCamera directly instead of abstract Camera class
...
protected:
    ...
    void SetUp() override {
        ...
        camera->setPosition(Math::WorldCoordinates(Math::Vector3f(0.0f, 0.0f, 5.0f)));
        camera->setTarget(Math::WorldCoordinates(Math::Vector3f(0.0f, 0.0f, 0.0f)));
        camera->setUp(Math::WorldCoordinates(Math::Vector3f(0.0f, 1.0f, 0.0f)));
