...
public:
    ...
    int eventCount = 0;
    Vector3f lastOldSize{0.0f, 0.0f, 0.0f};
    Vector3f lastNewSize{0.0f, 0.0f, 0.0f};
};
...
TEST_F(WorkspaceManagerTest, ValidSizeChanges) {
    ...
    for (const auto& newSize : validSizes) {
        int oldEventCount = resizeHandler->eventCount;
        
        EXPECT_TRUE(manager->setSize(newSize));
        EXPECT_EQ(manager->getSize(), newSize);
        
        // Basic event count check (events may have memory issues but count should work)
        EXPECT_EQ(resizeHandler->eventCount, oldEventCount + 1);
    }
    
    // Total event count should match number of size changes
    EXPECT_EQ(resizeHandler->eventCount, static_cast<int>(validSizes.size()));
}
...
TEST_F(WorkspaceManagerTest, MultipleSizeChanges) {
    ...
    // Total event count should match number of size changes
    EXPECT_EQ(resizeHandler->eventCount, static_cast<int>(sizes.size()));
}
