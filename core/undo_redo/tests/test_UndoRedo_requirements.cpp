...
public:
    TestCommand(int& value, int newValue, size_t memorySize = sizeof(TestCommand))
        : m_value(value)
        , m_newValue(newValue)
        , m_oldValue(value)
...
TEST_F(UndoRedoRequirementsTest, HistoryManager_OperationLimit) {
    ...
    history.setSnapshotInterval(0); // Disable snapshots for this test
    
    int value = 0;
    
    // Add 15 commands (exceeds limit)
    for (int i = 1; i <= 15; ++i) {
        EXPECT_TRUE(history.executeCommand(
            std::make_unique<TestCommand>(value, i)));
    }
    
    // Should only be able to undo 10 times
    int undoCount = 0;
    while (history.canUndo() && undoCount < 15) {
