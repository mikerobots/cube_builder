...
protected:
    ...
    void placeVoxel(const Vector3i& pos, VoxelResolution resolution) {
...
TEST_F(PlaneDetectorTest, DetectPlaneWithSingleVoxel) {
    ...
    placeVoxel(Vector3i(0, 0, 0), VoxelResolution::Size_32cm);
    ...
    EXPECT_EQ(result.plane.referenceVoxel, Vector3i(0, 0, 0));
...
TEST_F(PlaneDetectorTest, FindHighestVoxelUnderCursor) {
    ...
    placeVoxel(Vector3i(0, 0, 0), VoxelResolution::Size_32cm);  // Top height: 1 * 0.32 = 0.32m
    placeVoxel(Vector3i(0, 1, 0), VoxelResolution::Size_32cm);  // Top height: 2 * 0.32 = 0.64m
    placeVoxel(Vector3i(0, 4, 0), VoxelResolution::Size_16cm);  // Top height: 5 * 0.16 = 0.80m (highest)
    ...
    EXPECT_EQ(highestVoxelInfo.value().position, Vector3i(0, 4, 0));
...
TEST_F(PlaneDetectorTest, PlanePersistenceDuringOverlap) {
    ...
    Vector3i previewPos(1, 0, 0); // Adjacent to existing voxel
...
TEST_F(PlaneDetectorTest, PlaneClearingWhenPreviewClears) {
    ...
    Vector3i previewPos(10, 0, 10); // Far from existing voxel
    VoxelResolution previewRes = VoxelResolution::Size_32cm;
...
TEST_F(PlaneDetectorTest, DifferentVoxelSizes) {
    ...
    placeVoxel(Vector3i(0, 0, 0), VoxelResolution::Size_32cm);   // 32cm voxel at world (0, 0, 0)
    placeVoxel(Vector3i(2, 0, 0), VoxelResolution::Size_16cm);   // 16cm voxel at world (0.32, 0, 0)
    placeVoxel(Vector3i(6, 0, 0), VoxelResolution::Size_8cm);    // 8cm voxel at world (0.48, 0, 0)
...
TEST_F(PlaneDetectorTest, VoxelTopHeightCalculation) {
    float height32 = m_planeDetector->calculateVoxelTopHeight(Vector3i(0, 0, 0), VoxelResolution::Size_32cm);
    EXPECT_FLOAT_EQ(height32, 0.32f);
    
    float height16 = m_planeDetector->calculateVoxelTopHeight(Vector3i(0, 1, 0), VoxelResolution::Size_16cm);
    EXPECT_FLOAT_EQ(height16, 0.32f); // (1 + 1) * 0.16f = 0.32f
    
    float height8 = m_planeDetector->calculateVoxelTopHeight(Vector3i(0, 3, 0), VoxelResolution::Size_8cm);
    EXPECT_FLOAT_EQ(height8, 0.32f); // (3 + 1) * 0.08f = 0.32f
}

// Test plane transition logic
TEST_F(PlaneDetectorTest, PlaneTransitionLogic) {
    // Set current plane at 32cm height
    PlacementPlane currentPlane(0.32f, Vector3i(0, 0, 0), VoxelResolution::Size_32cm);
    m_planeDetector->setCurrentPlane(currentPlane);
    
    // Test transition to higher plane
    PlacementPlane higherPlane(0.64f, Vector3i(0, 1, 0), VoxelResolution::Size_32cm);
    ...
    PlacementPlane sameHeightPlane(0.32f, Vector3i(1, 0, 0), VoxelResolution::Size_32cm);
    ...
    PlacementPlane lowerPlane(0.16f, Vector3i(0, 0, 1), VoxelResolution::Size_16cm);
...
TEST_F(PlaneDetectorTest, VoxelsAtSpecificHeight) {
    // Place voxels at different heights
    placeVoxel(Vector3i(0, 0, 0), VoxelResolution::Size_32cm);  // Top at 0.32m
    placeVoxel(Vector3i(1, 0, 0), VoxelResolution::Size_32cm);  // Top at 0.32m
    placeVoxel(Vector3i(0, 1, 0), VoxelResolution::Size_32cm);  // Top at 0.64m
...
TEST_F(PlaneDetectorTest, PreviewOverlapDetection) {
    ...
    placeVoxel(Vector3i(0, 0, 0), VoxelResolution::Size_32cm);
    PlacementPlane plane(0.32f, Vector3i(0, 0, 0), VoxelResolution::Size_32cm);
    m_planeDetector->setCurrentPlane(plane);
    
    // Test overlapping preview
    bool overlaps = m_planeDetector->previewOverlapsCurrentPlane(Vector3i(0, 0, 0), VoxelResolution::Size_32cm);
    EXPECT_TRUE(overlaps);
    
    // Test non-overlapping preview
    bool noOverlap = m_planeDetector->previewOverlapsCurrentPlane(Vector3i(10, 0, 10), VoxelResolution::Size_32cm);
