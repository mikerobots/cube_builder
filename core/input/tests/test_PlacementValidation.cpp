...
TEST_F(PlacementValidationTest, SnapToValidIncrement) {
    // Test exact positions
    {
        ...
        Vector3i snapped = PlacementUtils::snapToValidIncrement(worldPos);
...
TEST_F(PlacementValidationTest, SnapToGridAligned) {
    // Test 32cm voxel without shift (should snap to 32cm grid)
    {
        ...
        Vector3i snapped = PlacementUtils::snapToGridAligned(worldPos, 
...
TEST_F(PlacementValidationTest, ValidateWorkspaceBounds) {
    ...
    {
        Vector3i gridPos(100, 100, 100); // 1m, 1m, 1m
        auto result = PlacementUtils::validatePlacement(gridPos, 
            VoxelData::VoxelResolution::Size_1cm, m_workspaceSize);
        EXPECT_EQ(result, PlacementValidationResult::Valid);
    }
    ...
    {
        Vector3i gridPos(-300, 100, 100); // -3m (outside -2.5m bound)
        auto result = PlacementUtils::validatePlacement(gridPos, 
            VoxelData::VoxelResolution::Size_1cm, m_workspaceSize);
    ...
    {
        Vector3i gridPos(100, 600, 100); // 6m (outside 5m height)
        auto result = PlacementUtils::validatePlacement(gridPos, 
            VoxelData::VoxelResolution::Size_1cm, m_workspaceSize);
        EXPECT_EQ(result, PlacementValidationResult::InvalidOutOfBounds);
    }
    ...
    {
        Vector3i gridPos(200, 100, 200); // 2m, 1m, 2m
        auto result = PlacementUtils::validatePlacement(gridPos, 
...
// Test world to increment grid conversion
TEST_F(PlacementValidationTest, WorldToIncrementGrid) {
    // Test positive positions
    {
        Vector3f worldPos(1.234f, 2.345f, 3.456f);
        Vector3i gridPos = PlacementUtils::worldToIncrementGrid(worldPos);
        EXPECT_EQ(gridPos.x, 123); // 1.234m = 123.4cm -> 123
        EXPECT_EQ(gridPos.y, 234); // 2.345m = 234.5cm -> 234
        EXPECT_EQ(gridPos.z, 345); // 3.456m = 345.6cm -> 345
    }
    ...
    // Test negative positions
    {
        Vector3f worldPos(-1.234f, -2.345f, -3.456f);
        Vector3i gridPos = PlacementUtils::worldToIncrementGrid(worldPos);
        EXPECT_EQ(gridPos.x, -124); // floor(-123.4) = -124
        EXPECT_EQ(gridPos.y, -235); // floor(-234.5) = -235
        EXPECT_EQ(gridPos.z, -346); // floor(-345.6) = -346
    }
}

// Test increment grid to world conversion
TEST_F(PlacementValidationTest, IncrementGridToWorld) {
    // Test positive grid positions
    {
        Vector3i gridPos(123, 234, 345);
        Vector3f worldPos = PlacementUtils::incrementGridToWorld(gridPos);
        EXPECT_FLOAT_EQ(worldPos.x, 1.23f);
        EXPECT_FLOAT_EQ(worldPos.y, 2.34f);
        EXPECT_FLOAT_EQ(worldPos.z, 3.45f);
    }
    
    // Test negative grid positions
    {
        Vector3i gridPos(-123, -234, -345);
        Vector3f worldPos = PlacementUtils::incrementGridToWorld(gridPos);
        EXPECT_FLOAT_EQ(worldPos.x, -1.23f);
...
TEST_F(PlacementValidationTest, GetPlacementContext) {
    // Test valid placement without shift
    {
        ...
        EXPECT_EQ(context.snappedGridPos.x, 128);  // Snaps to 1.28m (4*32cm)
        EXPECT_EQ(context.snappedGridPos.y, 64);   // Snaps to 0.64m (2*32cm)
        EXPECT_EQ(context.snappedGridPos.z, 128);  // Snaps to 1.28m (4*32cm)
    ...
    // Test valid placement with shift
    {
        ...
        EXPECT_EQ(context.snappedGridPos.x, 115); // 1.15m = 115cm
        EXPECT_EQ(context.snappedGridPos.y, 50);  // 0.5m = 50cm
        EXPECT_EQ(context.snappedGridPos.z, 115);
        EXPECT_EQ(context.validation, PlacementValidationResult::Valid);
...
TEST_F(PlacementValidationTest, ShiftKeyOverrideAllResolutions) {
    ...
    for (int i = 0; i < static_cast<int>(VoxelData::VoxelResolution::COUNT); ++i) {
        ...
        Vector3i snappedNoShift = PlacementUtils::snapToGridAligned(testPos, resolution, false);
        
        // With shift - should snap to 1cm increments
        Vector3i snappedWithShift = PlacementUtils::snapToGridAligned(testPos, resolution, true);
...
protected:
    void SetUp() override {
        ...
        m_dataManager->setVoxel(Vector3i(100, 0, 100), VoxelData::VoxelResolution::Size_32cm, true); // 32cm voxel at (1.0m, 0, 1.0m)
        m_dataManager->setVoxel(Vector3i(200, 0, 100), VoxelData::VoxelResolution::Size_32cm, true); // Adjacent 32cm voxel at (2.0m, 0, 1.0m) 
        m_dataManager->setVoxel(Vector3i(50, 0, 50), VoxelData::VoxelResolution::Size_16cm, true);   // 16cm voxel at (0.5m, 0, 0.5m)
    }
...
TEST_F(SmartSnappingTest, SameSizeVoxelSnapping) {
    ...
    Vector3i snapped = PlacementUtils::snapToSameSizeVoxel(worldPos, VoxelResolution::Size_32cm, *m_dataManager, false);
    
    // Should snap to 32cm grid (32cm = 0.32m = 32 increments)
    // 3.35m / 0.32m = 10.46875 rounds to 10, then 10 * 32 = 320
    Vector3i expected(320, 0, 320); // Grid-aligned to 32cm voxel size
...
TEST_F(SmartSnappingTest, SameSizeSnappingShiftOverride) {
    ...
    Vector3i snapped = PlacementUtils::snapToSameSizeVoxel(worldPos, VoxelResolution::Size_32cm, *m_dataManager, true);
    
    // With Shift pressed, should snap to 1cm increments regardless of nearby voxels
    Vector3i expected(335, 0, 335); // 3.35m rounded to nearest cm
    EXPECT_EQ(snapped.x, expected.x);
    EXPECT_EQ(snapped.y, expected.y);
    EXPECT_EQ(snapped.z, expected.z);
}
...
TEST_F(SmartSnappingTest, NoNearbyVoxelsSnapping) {
    ...
    Vector3i expected(704, 192, 704); // Grid-aligned for 32cm
    EXPECT_EQ(snapped.x, expected.x);
    EXPECT_EQ(snapped.y, expected.y);
    EXPECT_EQ(snapped.z, expected.z);
}
...
TEST_F(SmartSnappingTest, SurfaceFaceGridSnapping) {
    ...
    Math::Vector3i surfaceFaceVoxelPos(100, 0, 100); // 32cm voxel position (1.0m, 0, 1.0m)
    ...
    Vector3i snapped = PlacementUtils::snapToSurfaceFaceGrid(hitPoint, surfaceFaceVoxelPos, surfaceFaceRes, surfaceFaceDir, placementRes);
...
TEST_F(SmartSnappingTest, SurfaceFaceAllDirections) {
    ...
    Math::Vector3i voxelPos(100, 100, 100); // 32cm voxel at (1.0m, 1.0m, 1.0m)
    ...
    for (const auto& testCase : testCases) {
        Vector3i snapped = PlacementUtils::snapToSurfaceFaceGrid(testCase.hitPoint, voxelPos, voxelRes, testCase.direction, placementRes);
...
TEST_F(SmartSnappingTest, SmartPlacementContext) {
    ...
    Math::Vector3i surfaceFacePos(100, 0, 100);
    ...
    EXPECT_NE(contextNoFace.snappedGridPos.x, contextWithFace.snappedGridPos.x);
}
...
TEST_F(SmartSnappingTest, SurfaceFaceEdgeCases) {
    ...
    Math::Vector3i smallVoxelPos(50, 0, 50); // 16cm voxel at (0.5m, 0, 0.5m)
    ...
    Vector3i snapped = PlacementUtils::snapToSurfaceFaceGrid(hitPoint, smallVoxelPos, smallVoxelRes, faceDir, largeVoxelRes);
    ...
    Vector3f snappedWorld = PlacementUtils::incrementGridToWorld(snapped);
    ...
    for (auto resolution : resolutions) {
        ...
        for (float xPos : testPositions) {
            for (float zPos : testPositions) {
                ...
                EXPECT_EQ(context.snappedGridPos.x, static_cast<int>(std::round(xPos * 100)));
                EXPECT_EQ(context.snappedGridPos.y, 0);
                EXPECT_EQ(context.snappedGridPos.z, static_cast<int>(std::round(zPos * 100)));
            }
        }
    }
