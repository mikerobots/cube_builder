...
if timeout 10s "$CLI_PATH" --headless < "$TEST_DIR/ground_cmd.txt" > "$TEST_DIR/ground_output.txt" 2>&1; then
    ...
    PLACED_COUNT=$(grep -c "Voxel placed" "$TEST_DIR/ground_output.txt" || echo 0)
...
if timeout 10s "$CLI_PATH" --headless < "$TEST_DIR/overlap_cmd.txt" > "$TEST_DIR/overlap_output.txt" 2>&1; then
    PLACED_COUNT=$(grep -c "Voxel placed" "$TEST_DIR/overlap_output.txt" || echo 0)
    if [ "$PLACED_COUNT" -eq 1 ]; then
        pass_test "Overlap detection working (1 voxel placed, duplicates rejected)"
    else
        fail_test "Overlap detection not working (expected 1 placement, got $PLACED_COUNT)"
        echo "Output:"
...
# Test 3: 1cm increments (headless)
init_test "1cm Increment Positions"
...
if timeout 10s "$CLI_PATH" --headless < "$TEST_DIR/increment_cmd.txt" > "$TEST_DIR/increment_output.txt" 2>&1; then
    PLACED_COUNT=$(grep -c "Voxel placed" "$TEST_DIR/increment_output.txt" || echo 0)
    if [ "$PLACED_COUNT" -eq 3 ]; then
        pass_test "1cm increments working (3 voxels at 1cm intervals)"
    else
        fail_test "1cm increments not working (expected 3 placements, got $PLACED_COUNT)"
    fi
else
