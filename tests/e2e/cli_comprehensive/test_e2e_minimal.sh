...
echo "quit" | timeout 5s ../../../build_ninja/bin/VoxelEditor_CLI --headless > /tmp/cli_test1.txt 2>&1
if [ $? -eq 0 ]; then
...
timeout 10s ../../../build_ninja/bin/VoxelEditor_CLI --headless < /tmp/cli_test2_cmd.txt > /tmp/cli_test2.txt 2>&1
...
timeout 10s ../../../build_ninja/bin/VoxelEditor_CLI --headless < /tmp/cli_test3_cmd.txt > /tmp/cli_test3.txt 2>&1
if [ $? -eq 0 ]; then
...
EOF
    ...
    timeout 20s ../../../build_ninja/bin/VoxelEditor_CLI < /tmp/cli_test4_cmd.txt > /tmp/cli_test4.txt 2>&1
