...
if execute_cli_capture "$TEST_DIR/overlap_cmd.txt" "$TEST_DIR/overlap_output.txt" 10 true; then
    # Should have only 2 voxels (first at 64,64,64 and one at 128,128,128)
...
# Test 4: 1cm increment validation (actually tests overlap detection with rounded coords)
...
if execute_cli_capture "$TEST_DIR/increment_cmd.txt" "$TEST_DIR/increment_output.txt" 10 true; then
    # Should have 2 voxels: (0,0,0) and (16,16,16)
    # The 4.5cm placement rounds to 5cm, which overlaps with the 16cm voxel at (0,0,0)
    # The duplicate placement at (16,16,16) should also be rejected
    if expect_output "$TEST_DIR/increment_output.txt" "Voxels: 2"; then
        pass_test "Coordinate rounding and overlap detection working correctly"
    else
...
# Add many operations
for i in {0..150}; do
    echo "place $((i*16))cm 0cm $((i*16))cm" >> "$TEST_DIR/undo_limit_cmd.txt"
done
