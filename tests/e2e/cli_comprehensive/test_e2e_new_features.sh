...
resolution 16cm
place 64cm 0cm 64cm
place 112cm 0cm 112cm
place 128cm 0cm 128cm
place 144cm 0cm 144cm
zoom 0.5
...
if execute_cli_capture "$TEST_DIR/ground_constraint_cmd.txt" "$TEST_DIR/ground_output.txt" 30; then
    # Should only have 2 voxels (Y=0 and Y=64)
...
if execute_cli_capture "$TEST_DIR/overlap_test_cmd.txt" "$TEST_DIR/overlap_output.txt" 30; then
    # Should have 3 voxels (128,0,128), (64,0,64), (0,0,0) - no overlaps
    if expect_output "$TEST_DIR/overlap_output.txt" "Voxels: 3"; then
        pass_test "Overlap prevention working"
    else
        fail_test "Incorrect number of voxels placed"
    fi
else
