...
# Test immediate save after place - use exact positions to test new requirements
echo "Test: Place and Save (Exact Positions)"
cat > /tmp/storage_test.txt << EOF
workspace 5 5 5
resolution 64cm
place 197cm 3cm 203cm
save /tmp/test_voxels.vox
quit
EOF

rm -f /tmp/test_voxels.vox
timeout 10s ../../../build_ninja/bin/VoxelEditor_CLI --headless < /tmp/storage_test.txt 2>&1 | grep -E "placed|saved"
...
EOF
    ...
    timeout 10s ../../../build_ninja/bin/VoxelEditor_CLI --headless < /tmp/load_test.txt 2>&1 | grep -E "loaded|Voxels:"
else
    echo "❌ Save file not created"
fi

# Test with export - use exact positions to demonstrate new requirements
echo ""
echo "Test: Export to STL (Exact Positions)"
cat > /tmp/export_test.txt << EOF
workspace 5 5 5  
resolution 64cm
place 197cm 3cm 203cm
place 261cm 7cm 199cm
place 201cm 11cm 267cm
export /tmp/test_export.stl
quit
EOF

rm -f /tmp/test_export.stl
OUTPUT=$(timeout 10s ../../../build_ninja/bin/VoxelEditor_CLI --headless < /tmp/export_test.txt 2>&1)
