...
TEST_F(PlaceCommandTest, AllResolutions_LargeVoxelWorkspaceLimits_REQ_11_3_5) {
    ...
    std::vector<VoxelData::VoxelResolution> largeResolutions = {
        VoxelData::VoxelResolution::Size_256cm,  // 2.56m - should fit in 5m workspace
        VoxelData::VoxelResolution::Size_512cm,  // 5.12m - should NOT fit in 5m workspace
    };
    ...
    for (auto resolution : largeResolutions) {
        ...
        Math::IncrementCoordinates position(-100, 0, -100);  // -1m position, voxel extends to 1.56m
...
TEST_F(PlaceCommandTest, CollisionDetection_AdjacentVoxels_NoCollision_REQ_11_3_4) {
    ...
    std::vector<Math::IncrementCoordinates> adjacentPositions = {
        Math::IncrementCoordinates(16, 0, 0),   // Adjacent in +X direction
        Math::IncrementCoordinates(-16, 0, 0),  // Adjacent in -X direction
        Math::IncrementCoordinates(0, 16, 0),   // Adjacent in +Y direction
        Math::IncrementCoordinates(0, 0, 16),   // Adjacent in +Z direction
        Math::IncrementCoordinates(0, 0, -16),  // Adjacent in -Z direction
    };
...
TEST_F(PlaceCommandTest, CollisionDetection_MultipleResolutions_REQ_11_3_4) {
    ...
    std::vector<std::pair<VoxelData::VoxelResolution, Math::IncrementCoordinates>> testVoxels = {
        {VoxelData::VoxelResolution::Size_1cm, Math::IncrementCoordinates(32, 0, 32)},
        {VoxelData::VoxelResolution::Size_4cm, Math::IncrementCoordinates(64, 0, 64)},
        {VoxelData::VoxelResolution::Size_4cm, Math::IncrementCoordinates(96, 0, 96)},
        {VoxelData::VoxelResolution::Size_16cm, Math::IncrementCoordinates(128, 0, 128)},
        {VoxelData::VoxelResolution::Size_16cm, Math::IncrementCoordinates(160, 0, 160)},
    };
...
TEST_F(PlaceCommandTest, CollisionDetection_LargeVoxelOverlap_REQ_11_3_4) {
    ...
    Math::IncrementCoordinates smallPos(0, 0, 0);
    ...
    Math::IncrementCoordinates largePos(0, 0, 0);
    ...
    Math::IncrementCoordinates noOverlapPos(64, 0, 64);  // Far enough away
...
TEST_F(PlaceCommandTest, CollisionDetection_EdgeCases_REQ_11_3_4) {
    ...
    Math::IncrementCoordinates position(40, 0, 40);
...
TEST_F(PlaceCommandTest, PlacementCommandFactory_UsesVoxelDataManagerValidation_REQ_11_3_3) {
    // Test that PlacementCommandFactory properly uses VoxelDataManager's validation
    // The actual extent validation logic is tested in voxel_data unit tests
    
    EXPECT_TRUE(initialized) << "Application must be initialized for this test";
    
    // Test a position that's clearly outside bounds
    Math::IncrementCoordinates outOfBounds(1000, 0, 1000);  // Way outside 5m workspace
    
    auto validation = UndoRedo::PlacementCommandFactory::validatePlacement(
        voxelManager.get(), outOfBounds, VoxelData::VoxelResolution::Size_1cm);
    
    EXPECT_FALSE(validation.valid) 
        << "PlacementCommandFactory should reject out-of-bounds positions";
    
    EXPECT_FALSE(validation.errors.empty()) 
        << "Should provide error message";
    
    // Test that commands aren't created for invalid positions
    auto command = UndoRedo::PlacementCommandFactory::createPlacementCommand(
        voxelManager.get(), outOfBounds, VoxelData::VoxelResolution::Size_1cm);
    
    EXPECT_EQ(command, nullptr)
        << "Should not create command for invalid position";
}
