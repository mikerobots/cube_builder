...
(
    ...
    echo "resolution 8cm" 
    echo "place 0cm 8cm 0cm"  # Center voxel (Y must be >= 0)
    echo "place 8cm 8cm 0cm"
    echo "place 0cm 16cm 0cm"
    echo "place 0cm 8cm 8cm"
    echo "camera iso"
    sleep 1
...
EOF
    ...
    (
        ...
        echo "place -16cm 0cm -16cm"  # Negative X and Z coordinates
        echo "place 0cm 0cm 0cm"    # Origin
        echo "place 16cm 0cm 16cm"    # Positive coordinates
        echo "camera iso"
        sleep 1
    ...
    CLEAR_FILE="$TEST_OUTPUT/test_clear.ppm"
    if [ ! -f "$CLEAR_FILE" ] && [ -f "$CLEAR_FILE.ppm" ]; then
        CLEAR_FILE="$CLEAR_FILE.ppm"
    fi
...
EOF
    ...
    CENTERED_FILE="$TEST_OUTPUT/test_centered.ppm"
    if [ ! -f "$CENTERED_FILE" ] && [ -f "$CENTERED_FILE.ppm" ]; then
        CENTERED_FILE="$CENTERED_FILE.ppm"
    fi
    
    if [ -f "$CENTERED_FILE" ]; then
        echo "✓ Centered coordinate screenshot created"
        python3 - <<EOF
with open('$CENTERED_FILE', 'rb') as f:
    # Skip header
    magic = f.readline()
